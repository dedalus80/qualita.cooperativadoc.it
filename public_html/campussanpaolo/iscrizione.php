...
$q = $db->FetchArray($db->Query("SELECT * FROM ca_preiscrizioni WHERE id='" . $ID . "'"));
...
if ($q) {
    ...
    $nf = $db->SingleField("MAX(id_iscrizione)", "ca_preiscrizioni", " WHERE 1 ") + 1;
    $db->Query("UPDATE  ca_preiscrizioni SET id_iscrizione ='" . $nf . "' WHERE id='" . $ID . "'");
    ...
    class PDF extends FPDF {
        ...
        function Header() {
            $this->Image('./img/logo_campussanpaolo.jpg', 120, 12, '70');
        ...
        function Footer() {
            ...
            $this->Cell(0, 0, 'CAMPUS SANPAOLO è una gestione Sharing s.r.l | Via Assietta 15/f 10128 Torino  | P.iva e C.F 10442110010 T +39.0112243024 F +39.0112243041', 0, 0, 'C');
        }
    ...
    $pdf->Ln(5);
    $pdf->SetFont('Arial', '', 11);
    $pdf->Cell(100, 5, $db->SingleField("nome_" . str_replace("-", "_", $lang), "doc_formule", "WHERE id='1'"), 0);
    $pdf->Cell(100, 5, $db->SingleField("nome_" . str_replace("-", "_", $lang), "doc_camere", "WHERE id='" . $q['campus'] . "'"), 0);
    ...
    $pdf->Output("./tmp/preiscrizione_campussanpaolo_" . $nf . ".pdf", "F");
    chmod("./tmp/preiscrizione_campussanpaolo_" . $nf . ".pdf", 0777);
	...
	$dest = array (
        ...
        "1" => array("nome"=> "Booking CampusSanPaolo.it" , "email" => "booking@campussanpaolo.it"), 
	...
	foreach($dest as $recipient) {
		...
		$mail->SMTPAuth = true;
		$mail->SMTPSecure = 'tls';
		$mail->Username = "pre-iscrizioni@campussanpaolo.it";
		$mail->Password = "Juxa6249";
		$mail->Subject = $oggetto;
		$mail->setFrom('pre-iscrizioni@campussanpaolo.it', 'Booking CampusSanPaolo.it');
		$mail->addReplyTo('pre-iscrizioni@campussanpaolo.it', 'Booking CampusSanPaolo.it');
    ...
    unlink("./tmp/preiscrizione_campussanpaolo_" . $nf . ".pdf");
}
?>