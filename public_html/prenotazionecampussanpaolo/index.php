...
//~ require_once(ABSPATH . 'wp-register/includes/lang.php');
	...
	$formule        = $db->CycleAssochId($db->Query("SELECT id, nome_" .LANG. " as nome FROM doc_formule WHERE form_campus = 'Y' AND show_on_q = 'Y'"));
	...
	$facolta = $db->CycleAssochId($db->Query("SELECT id, nome_facolta as nome FROM facolta ORDER BY id"));
...
<html lang="en">
    <head>
        ...
        <title>Campus Sanpaolo ::: modulo prenotazioni</title>
		...
		<style>
			...
			body {
				background: url(assets/img/back_campus.jpg) no-repeat center center fixed;
    ...
    <body>
		<div class="container">
			<div class="row py-5">
				<div class="col-md-2">
					<img class="img-responsive" style="width:100px;" src="./assets/img/logo_campus.png">
				</div>
				<div class="col-md-10 my-auto">
					<h4>Prenota al Campus Sanpaolo</h4>
					<p class="lead">Compila il form per prenotare il tuo appartamento al Campus Sanpaolo!</p>
				</div>
			</div>
			...
			<div class="row">
				...
				<div class="col-xl-12">
					<form id="form-campus" method="POST">
						<input type="hidden" name="language" id="language" value="<?= LANG ?>" />
						<input type="hidden" name="from" id="from" value="campus" />
						...
						<div id="form-dati">
							...
							<div class="form-group row">
								<div class="col-sm-6">
									<label for="facolta"><?= $text[LANG]['label_facolta'] ?>*</label>
                                    <select name="facolta" id="facolta" class="form-control obbligatorio valida" data-label="<?= $text[LANG]['label_facolta'] ?>">
										<option value=""><?= $text[LANG]['label_scegli'] ?></option>
										<?php foreach ($facolta as $id => $val): ?>
										<option value="<?= $id ?>"><?= $val ?></option>
                                        <?php endforeach; ?>
                                    </select>
								</div>
								<div class="col-sm-6">
									<label><?= $text[LANG]['label_sesso'] ?>*</label>
									<div>
						...
						<div id="form-richiesta">
                            ...
                            <div class="form-group row">
                                <div class="col-sm-6">
                                    <label for="conoscienza"><?= $text[LANG]['label_conoscenza_campus'] ?>*</label>
									<select name="conoscienza" id="conoscienza" class="form-control obbligatorio valida" data-label="<?= $text[LANG]['label_conoscenza_campus'] ?>">
										<option value=""><?= $text[LANG]['label_scegli'] ?></option>
										<?php foreach ($conoscienza as $id => $val): ?>
											<option value="<?= $id ?>"><?= utf8_encode($val) ?></option>
										<?php endforeach; ?>
									</select>
                                </div>
                                <div class="col-sm-6">
									<label><?= $text[LANG]['label_prima_volta_campus'] ?>*</label>
									<div>
										<div class="form-check form-check-inline">
											<input name="prima_volta" id="prima_volta_y" value="Y" class="form-check-input check-error" type="radio" data-label="<?= $text[LANG]['label_prima_volta_campus'] ?>">
											<label class="form-check-label" for="prima_volta_y"><?= $text[LANG]['label_si'] ?></label>
										</div>
										<div class="form-check form-check-inline">
											<input name="prima_volta" id="prima_volta_n" value="N" class="form-check-input check-error" type="radio" data-label="<?= $text[LANG]['label_prima_volta_campus'] ?>">
							...
							<div class="form-group row">
								<div class="col-sm-6">
									...
									<select name="campus" id="campus" class="form-control" data-label="<?= $text[LANG]['label_campus'] ?>" data-tipo="number">
										<option value=""><?= $text[LANG]['label_scegli'] ?></option>
										<?php foreach ($camere as $id => $val): ?>
											<option value="<?= $id ?>"><?= utf8_encode($val) ?></option>
										<?php endforeach; ?>
									</select>
								</div>
						...
						<div class="row mt-3">
							<div class="col-sm-12 text-right">
								<div class="box-button">
									<a href="#" class="button" id="btn-submit-form" data-refer="form-campus">
