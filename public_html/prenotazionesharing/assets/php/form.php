<?php
	...
	if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
		...
		$campi    = array('nazionalita', 'occupazione', 'conoscienza','cellulare', 'nome', 'cognome', 'luogo_nascita','email','formula');
...
//~ 			 $error .= str_replace("[CAMPO]",$val,$lingua['error_nonvalido']);
        ...
        //if($q['note'] != '')
        //    $q['note'] = filter_var($q['note'], FILTER_FLAG_NO_ENCODE_QUOTES | FILTER_FLAG_STRIP_LOW | FILTER_FLAG_STRIP_HIGH | FILTER_FLAG_STRIP_BACKTICK);
		...
		if (!$error) {
			...
			$query = "INSERT INTO ".$table." (nome, cognome, data_nascita, luogo_nascita, nazionalita, sesso, email, cellulare, occupazione, prima_volta, conoscenza, formula, campus, housing, coabitazione, data_in, data_out, privacy, mailing, note, data_insert,lang , anno,refer)
					...
					'" . reverseDate($q['data_partenza']) . "', '" . $q['privacy'] . "', '" . $q['mailing'] . "', '" . $db->escapeString($q['note']) . "', NOW() ,'" . str_replace("_","-",$q['language']) . "' ,'".date("Y")."','".$referer."' )";
