...
class AzioniVerificheEducazioneController extends Controller {
    ...
    public function actionModello() {
        $model = new AzioniVerificheEducazione('search');
        $model->setDefaultValue();
        $html2pdf = Yii::app()->ePdf->HTML2PDF('P', 'A4', 'en', false, 'ISO-8859-15', array(mL, mT, mR, mB));
        $html2pdf->WriteHTML($this->renderPartial('_modello', array('model' => $model), true));
        $html2pdf->Output('modello-verifica-ispettiva-educazione.pdf', 'D');
    }

    public function actionCreate() {
        $model = new AzioniVerificheEducazione;
        $model->setDefaultValue();
        if (isset($_POST['AzioniVerificheEducazione'])) {

            $model->attributes = $_POST['AzioniVerificheEducazione'];
    ...
    public function actionIndex() {
        $dataProvider = new CActiveDataProvider('AzioniVerificheEducazione');
    ...
    public function loadModel($id) {
        $model = AzioniVerificheEducazione::model()->findByPk($id);
        if ($model === null)
    ...
    protected function performAjaxValidation($model) {
        if (isset($_POST['ajax']) && $_POST['ajax'] === 'azioni-verifiche-educazione-form') {
