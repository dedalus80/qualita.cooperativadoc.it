...
class AzioniVerificheSicurezzaController extends Controller {
    ...
    public function actionModello() {
        $model = new AzioniVerificheSicurezza('search');
        $model->setDefaultValue();
        $html2pdf = Yii::app()->ePdf->HTML2PDF('P', 'A4', 'en', false, 'ISO-8859-15', array(mL, mT, mR, mB));
        $html2pdf->WriteHTML($this->renderPartial('_modello', array('model' => $model), true));
        $html2pdf->Output('modello-verifica-ispettiva-sicurezza.pdf', 'D');
    }
    ...
    public function actionCreate() {
        $model = new AzioniVerificheSicurezza;
        $model->setDefaultValue();
        if (isset($_POST['AzioniVerificheSicurezza'])) {

            $model->attributes = $_POST['AzioniVerificheSicurezza'];
            ...
            if ($model->save()) {
                ...
                Yii::app()->user->setFlash('opResultOK', 'Verifica sicurezza  <b>' . $model->codice_verifica . "</b> creata con successo");
    ...
    public function actionUpdate($id) {
        ...
        if (isset($_POST['AzioniVerificheSicurezza'])) {
            ...
            if ($model->save()) {
                ...
                Yii::app()->user->setFlash('opResultOK', 'Verifica sicurezza  <b>' . $model->codice_verifica . "</b> aggiornata con successo");
                $this->redirect(array('admin'));
            }
        }
    ...
    public function actionDelete($id) {
        $model = $this->loadModel($id);
        Yii::app()->user->setFlash('opResultOK', 'Verifica sicurezza  <b>' . $model->codice_verifica . "</b> rimossa con successo");
    ...
    public function actionIndex() {
        $dataProvider = new CActiveDataProvider('AzioniVerificheSicurezza');
    ...
    public function loadModel($id) {
        $model = AzioniVerificheSicurezza::model()->findByPk($id);
        if ($model === null)
    ...
    protected function performAjaxValidation($model) {
        if (isset($_POST['ajax']) && $_POST['ajax'] === 'azioni-verifiche-sicurezza-form') {
