...
class DocumentsProcedures extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('id, category_id, procedura', 'safe', 'on'=>'search'),
		);
	}
	...
	protected function beforeSave()
	{
		if (parent::beforeSave()) {
			// Se category_id non è già impostato, prova a prenderlo dalla richiesta
			if (empty($this->category_id)) {
				// Prova prima da GET, poi da POST
				$categoryId = Yii::app()->request->getQuery('category_id');
				if (empty($categoryId)) {
					$categoryId = Yii::app()->request->getPost('category_id');
				}
				// Se è presente nei dati POST del modello stesso
				if (empty($categoryId) && isset($_POST['DocumentsProcedures']['category_id'])) {
					$categoryId = $_POST['DocumentsProcedures']['category_id'];
				}
