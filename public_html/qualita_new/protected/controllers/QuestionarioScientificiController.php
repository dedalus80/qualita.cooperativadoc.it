...
class QuestionarioScientificiController extends Controller {
    ...
    public function accessRules() {
        return array(
            array('allow', // allow authenticated user to perform 'create' and 'update' actions
                ...
                'users' => Yii::app()->MyUtils->getPermition('q_scientifici'),
            ),
    ...
    public function actionGrafici($struttura= null, $anno = null, $gruppo = null, $turno = null) {
        ...
        $model = new QuestionarioScientifici;
        $anno ?  $model->anno = $anno : $model->anno = Yii::app()->MyStats->getCountStatsQuestionari("questionario_scientifici") ;
        $model->selectSoggiorni     = Yii::app()->MyUtils->getSoggiorniQ("questionario_scientifici",'soggiorno');
        $model->selectAnni          = Yii::app()->MyUtils->getYearsQ("questionario_scientifici");
        $model->stats               = Yii::app()->MyStats->getStatsQuestionari("questionario_scientifici", $struttura,$model->anno);
    ...
    /*public function actionStampaGrafici() {
        ...
        $model->stats   = Yii::app()->MyStats->getStatsQuestionari("questionario_scientifici", $struttura,$anno);
        ...
        $html2pdf->Output( YiiBase::getPathOfAlias('webroot').'/protected/stampe/questionari_scientifici/'.$file.'.pdf', 'F');
        header('Content-Type: application/json; charset="UTF-8"');
        echo CJSON::encode(array('stampa' => '/protected/stampe/questionari_scientifici/'.$file.'.pdf?ver='.time()));
        Yii::app()->end();
    }*/
    ...
    public function actionEsporta($anni = null) {
        $model = new QuestionarioScientifici('search');
    ...
    public function actionUpdate($id) {
        ...
        if (isset($_POST['QuestionarioScientifici'])) {
            $model->attributes = $_POST['QuestionarioScientifici'];
            $model->setAttribute('data_restituzione', Yii::app()->MyUtils->reverseDate($model->data_restituzione));
            if ($model->save()) {
                Yii::app()->user->setFlash('opResultOK', 'Questionario campus formativi <b>' . $model->nome . " " . $model->cognome . "</b> aggiornato con successo");
    ...
    public function loadModel($id) {
        $model = QuestionarioScientifici::model()->findByPk($id);
        if ($model === null)
    ...
    protected function performAjaxValidation($model) {
        if (isset($_POST['ajax']) && $_POST['ajax'] === 'questionario-scientifici-form') {
