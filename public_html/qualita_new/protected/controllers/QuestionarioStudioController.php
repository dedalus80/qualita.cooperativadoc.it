...
class QuestionarioStudioController extends Controller {
    ...
    public function accessRules() {
        return array(
            array('allow', // allow authenticated user to perform 'create' and 'update' actions
                ...
                'users' => Yii::app()->MyUtils->getPermition('q_studio'),
            ),
    ...
    public function actionGrafici($struttura= null, $anno = null, $gruppo = null, $turno = null) {
        ...
        $model = new QuestionarioStudio;
        $anno ?  $model->anno = $anno : $model->anno = Yii::app()->MyStats->getCountStatsQuestionari("questionario_studio") ;
        $model->selectSoggiorni     = Yii::app()->MyUtils->getSoggiorniQ("questionario_studio",'soggiorno');
        $model->selectAnni          = Yii::app()->MyUtils->getYearsQ("questionario_studio");
        $model->stats               = Yii::app()->MyStats->getStatsQuestionari("questionario_studio", $struttura,$model->anno);
    ...
    /*public function actionStampaGrafici() {
        ...
        $model->stats               = Yii::app()->MyStats->getStatsQuestionari("questionario_studio", $struttura,$anno);
        ...
        $html2pdf->Output( YiiBase::getPathOfAlias('webroot').'/protected/stampe/questionari_studio/'.$file.'.pdf', 'F');
        header('Content-Type: application/json; charset="UTF-8"');
        echo CJSON::encode(array('stampa' => '/protected/stampe/questionari_studio/'.$file.'.pdf?ver='.time()));
        Yii::app()->end();
    }*/
    ...
    public function actionUpdate($id) {
        ...
        $model->studio_attivita_utile = html_entity_decode($model->studio_attivita_utile);
        $model->studio_aspetto_vacanza = html_entity_decode($model->studio_aspetto_vacanza);


        $this->performAjaxValidation($model);
        $model->setSelect();

        if (isset($_POST['QuestionarioStudio'])) {
            $model->attributes = $_POST['QuestionarioStudio'];
            $model->setAttribute('data_restituzione', Yii::app()->MyUtils->reverseDate($model->data_restituzione));
            if ($model->save()) {
                Yii::app()->user->setFlash('opResultOK', 'Questionario vacanze studio <b>' . $model->nome . " " . $model->cognome . "</b> aggiornato con successo");
    ...
    public function actionEsporta($anni = null) {
        $model = new QuestionarioStudio('search');
    ...
    public function loadModel($id) {
        $model = QuestionarioStudio::model()->findByPk($id);
        if ($model === null)
    ...
    protected function performAjaxValidation($model) {
        if (isset($_POST['ajax']) && $_POST['ajax'] === 'questionario-studio-form') {
