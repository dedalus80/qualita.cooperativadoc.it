...
/**
 * This is the model class for table "doc_documenti_soggiorni".
 *
 * The followings are the available columns in table 'doc_documenti_soggiorni':
 * @property integer $id
 ...
 * @property Funzione $funzioneResponsabile
 * @property DocumentiSoggiorniProcedura $procedura
 * @property DocumentiSoggiorniUnita[] $DocumentiSoggiorniUnitas
 */
class DocumentiSoggiorni extends CActiveRecord
{
	public $document;
	 ...
	 * @return DocumentiSoggiorni the static model class
	 */
	...
	{
		return 'doc_documenti_soggiorni';
	}
	...
	public function rules()
	{
		...
		return array(
			array('funzione_responsabile_id, sgq, tipologia, codice, numero, revisione, data_revisione, titolo, redige, archivia, riesamina, autorizza, approva, periodicita_riesame, modalita_archiviazione, luogo_archiviazione', 'required'),
			array('procedura_id, funzione_responsabile_id, creato_user_id, modificato_user_id', 'numerical', 'integerOnly'=>true),
	...
	public function relations()
	{
		...
		return array(
			...
			'procedura' => array(self::BELONGS_TO, 'DocumentiSoggiorniProcedura', 'procedura_id'),
			'DocumentiSoggiorniUnitas' => array(self::HAS_MANY, 'DocumentiSoggiorniUnita', 'documenti_id'),
		);
	}
	...
	public function attributeLabels()
	{
		return array(
			...
			'creato_user_id' => 'Creato da utente',
			'modificato_user_id' => 'Modificato da utente',
			'document' => 'File',
		);
	}
	 ...
	 */
	public function search($proceduraId = null)
	{
		...
		//$criteria->mergeWith($this->dateRangeSearchCriteria('data_inserimento', $this->data_inserimento));
	...
	public function getNomeAllegato($ext)
	{
		if($this->procedura_id && $this->titolo && $this->codice && $this->numero) {
			$name = $this->procedura_id."_".$this->codice."_".$this->numero."_".time().".".$ext;
			return $name;
		}
	...
	{
		if($this->scenario == 'update' && $this->document){
			@unlink(Yii::app()->basePath.'/documenti_soggiorni/'.$this->oldFilename);
		}
	}
}