...
class QuestionarioDoc extends CActiveRecord {
    ...
    public function attributeLabels() {
        return array(
            'id' => 'ID',
            'nome' => 'Nome Compilatore',
            'cognome' => 'Cognome Compilatore',
            'data_consegna' => 'Data Consegna',
            'data_restituzione' => 'Data Restituzione',
            'vacanza' => 'Giudizio<span class="hidden-480"> Complessivo</span>',
            'struttura_nome' => 'Struttura',
    ...
    public function search() {
        ...
        $criteria->compare('struttura_nome', $this->struttura_nome, true);
    ...
    public function getStats($struttura = null, $anno = null) {
                
        
        
        
        $anno ? $this->anno = $anno : $this->anno =  date("Y");
        $ieri = $this->anno - 1;
        ...
        if($struttura){
            ...
            $this->struttura_nome   = Yii::app()->MyUtils->getSelectValue($struttura, "doc_unita");
        
        }else 
            $AND                    .= " AND struttura_nome !=''";
            
         
        $gen['anno']      = Yii::app()->db->createCommand("SELECT count(id) FROM  " . $this->tableName() . " WHERE  ".$WHERE)->queryScalar();
        $gen['ieri']      = Yii::app()->db->createCommand("SELECT count(id) FROM  " . $this->tableName() . " WHERE ".$WHEREIERI)->queryScalar();
        $gen['struttura'] = Yii::app()->db->createCommand("SELECT count(id) FROM  " . $this->tableName() . " WHERE ".$WHERE." ".$AND)->queryScalar();
        $gen['struttura_ieri'] = Yii::app()->db->createCommand("SELECT count(id) FROM  " . $this->tableName() . " WHERE ".$WHEREIERI." ".$AND)->queryScalar();
        
        $valori = array("I","S","B","E");
        $giudizzi =array(
            'vacanza', 'struttura_pulizia','struttura_complessivo','stanza_confort','stanza_arredi','stanza_pulizia','stanza_complessivo','ristorante_servizio',
            'ristorante_attesa','ristorante_cibo','ristorante_menu','ristorante_complessivo','personale_cortesia','personale_professionalita','personale_complessivo');
        
        foreach ($giudizzi as $giudizio) {
            
            $dati[$giudizio] = array();
            
            $dati[$giudizio]['anno']            = array();
            $dati[$giudizio]['precedente']      = array();
            $dati[$giudizio]['totale']          = array();
            $dati[$giudizio]['anno_per']        = array();
            $dati[$giudizio]['precedente_per']  = array();
            $dati[$giudizio]['totale_per']      = array();    

            foreach($valori as $valore ){
                
                $query      = "SELECT count(id) FROM ".$this->tableName()." WHERE ".$giudizio."='".$valore."' ";
                
                $totale     = Yii::app()->db->createCommand($query." AND ".$WHERE)->queryScalar();
                $anno       = Yii::app()->db->createCommand($query." AND ".$WHERE." ".$AND)->queryScalar();
                $precedente = Yii::app()->db->createCommand($query." AND ".$WHEREIERI." ".$AND)->queryScalar();
                                
                $dati[$giudizio]['totale'][]    = $totale ;
                $dati[$giudizio]['anno'][]      = $anno; 
                $dati[$giudizio]['precedente'][]  = $precedente;
                
                $dati[$giudizio]['totale_per'][] = $this->getPercent($totale, $gen['anno']);
                $dati[$giudizio]['anno_per'][]   = $this->getPercent($anno, $gen['struttura']);
                $dati[$giudizio]['precedente_per'][] = $this->getPercent($precedente, $gen['struttura_ieri']);
                
            }
        }
        
        $dati['totale'] = $gen['struttura'];
        $this->stats = $dati;
        return $dati;
    }
