...
class UtenzeAcqua extends CActiveRecord {
    ...
    public function getEsportazione($anno = NULL) {
        ...
        $query = " SELECT r.* , u.nome as nome_unita FROM utenze_acqua as r LEFT JOIN doc_unita  AS u ON r.struttura = u.id   " . $WHERE;
        ...
        for ($x = 0; $x < count($dati); $x++) {
            ...
            $uperficie =  Yii::app()->MyUtils->getSelectValue($dati[$x]['struttura'], "doc_unita_superficie");
            if ($dati[$x]['presenze'] > 0){
                $dati[$x]['media'] = $dati[$x]['totale'] / $dati[$x]['presenze'];
                $dati[$x]['c_media'] = $dati[$x]['c_totale'] / $dati[$x]['presenze'];
                $uperficie > 0 ? $utenze[$id . '_media_superficie'] = $consumi["c_" . $id] / $superficie: $utenze[$id.'_media_superficie'] = 0;
                $uperficie > 0 ? $utenze[$id . '_media_superficie_unita'] = $consumi[$id] / $superficie : $utenze[$id.'_media_superficie_unita'] = 0;
