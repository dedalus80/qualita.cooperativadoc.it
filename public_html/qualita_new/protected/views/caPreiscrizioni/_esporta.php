...
$objPHPExcel->getProperties()->setCreator("Cooperativa doc")->setTitle("San Paolo Preiscrizione");
...
$label = array(
    'Refer','Data', 'ID', 'Dal', 'Al', 'Formula','Stanza', 'Nome', 'Cognome', 'Cellulare', 'Email', 'Sesso', 'Data Nascita', 'Luogo Nascita', 'Nazionalita', 'Facolta',
...
for ($x = 0; $x < count($model->datiEsportazione); $x++) {
    ...
    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(0, $row, $model->datiEsportazione[$x]['refer'] =='S' ? "Campusanpaolo":"Politecnico");
    ...
    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(15, $row, $model->datiEsportazione[$x]['nome_facolta']);
    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(16, $row, $model->datiEsportazione[$x]['nome_occupazione']);
    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(17, $row, $model->datiEsportazione[$x]['prima_volta'] == 'Y' ? "SI" : "NO");
    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(18, $row, $model->datiEsportazione[$x]['nome_conoscenza']);
    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(19, $row, $model->datiEsportazione[$x]['coabitazione']);
    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(20, $row, $model->datiEsportazione[$x]['privacy'] == 'Y' ? "SI" : "NO");
    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(21, $row, $model->datiEsportazione[$x]['mailing'] == 'Y' ? "SI" : "NO");
    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(22, $row, utf8_decode($model->datiEsportazione[$x]['note']));
      $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(23, $row, utf8_decode($model->datiEsportazione[$x]['anno']));
    $row++;
}

header('Content-Type: application/vnd.ms-excel');
header('Content-Disposition: attachment;filename="Preiscrizioni_CampuSanPaolo.xls"');
