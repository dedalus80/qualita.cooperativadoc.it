<?php
/* @var $this DocumentiSoggiorniProceduraController */
/* @var $model DocumentiSoggiorniProcedura */

$this->breadcrumbs=array(
	'Documenti Qualita Proceduras'=>array('index'),
	$model->id,
);

$this->menu=array(
	array('label'=>'List DocumentiSoggiorniProcedura', 'url'=>array('index')),
	array('label'=>'Create DocumentiSoggiorniProcedura', 'url'=>array('create')),
	array('label'=>'Update DocumentiSoggiorniProcedura', 'url'=>array('update', 'id'=>$model->id)),
	array('label'=>'Delete DocumentiSoggiorniProcedura', 'url'=>'#', 'linkOptions'=>array('submit'=>array('delete','id'=>$model->id),'confirm'=>'Are you sure you want to delete this item?')),
	array('label'=>'Manage DocumentiSoggiorniProcedura', 'url'=>array('admin')),
);
?>

<h1>View DocumentiSoggiorniProcedura #<?php echo $model->id; ?></h1>

<?php $this->widget('zii.widgets.CDetailView', array(
	'data'=>$model,
	'attributes'=>array(
		'id',
		'procedura',
	),
)); ?>
