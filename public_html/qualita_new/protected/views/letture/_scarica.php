...
$objPHPExcel->getProperties()->setCreator("Cooperativa doc")->setTitle("Consumi Energetici");
$style1 = array(
    'font' => array(
        'bold' => false,
...
$objPHPExcel->getActiveSheet()->setTitle("Consumi Gas ");
...
$row = 1;
$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(0, $row, "CONSUMI GAS " . strtoupper($model->struttura_nome) . " " . $model->anno);
$row++;
foreach ($label AS $id => $val)
    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($id, $row, iconv('windows-1251', 'utf-8', $val));

$row++;
$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(0, $row,"MC" );
...
$row++;
$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(0, $row,"Presenze" );
foreach($mesi as $id => $val){
     $x = "c_".$mese;
...
$row++;
$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(0, $row,"Costo medio Euro" );
foreach($mesi as $id => $val){
     $x = "c_".$mese;
...
$row++;
$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(0, $row,"Consumo medio MC" );
foreach($mesi as $id => $val){
     $x = "c_".$mese;
...
header('Content-Disposition: attachment;filename="Consumi_Gas_'.str_replace(" ","_",$model->struttura_nome).'_'.$model->anno.'.xls"');
