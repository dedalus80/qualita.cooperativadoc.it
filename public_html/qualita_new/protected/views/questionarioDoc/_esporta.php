...
$objPHPExcel->getActiveSheet()->getStyle('A1:AC1')->applyFromArray($style1);
for ($x = 2; $x < count($model->datiEsportazione) + 2; $x++) {
    if ($x % 2 == 0)
        $objPHPExcel->getActiveSheet()->getStyle('A' . $x . ':AC' . $x)->applyFromArray($style3);
}
...
$label = array('ID', 'Data restituzione', 'Nome', 'Cognome','Email','Cellulare','Arrivo','Partenza','Giorni permanenza','Tipo cliente','Conoscenza', 'La Vacanza', 'Struttura', 'Struttura Pulizia', 'Struttura Complessivo', 'Stanza Confort', 'Stanza Arredi', 'Stanza Pulizia',
    ...
    'Personale Cortesia', 'Personale professionalita\'', 'Personale Complessivo', 'Consiglia', 'Suggerimenti'
);
...
for ($x = 0; $x < count($model->datiEsportazione); $x++) {
    ...
    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(27, $row, $model->datiEsportazione[$x]['nome_consiglia']);
    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(28, $row, $model->datiEsportazione[$x]['suggerimenti']);
