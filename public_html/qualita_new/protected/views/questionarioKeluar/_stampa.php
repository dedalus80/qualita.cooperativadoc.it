...
foreach ($model->attributes as $id => $val)
    $stats[$id] = $model->getStats($id,$struttura ? $struttura:"", $periodo);

$consiglia = $model->getConsiglia($model->attributes['consiglia'],$struttura ? $struttura:"", $periodo);
?>

<h1>Statistiche Questionari Keluar <span class='red'> <?= $nomeStruttura ? "".$nomeStruttura:""?> </span> <?= $periodo ? "Periodo: ".$model->getPeriodo($periodo):""?> </h1>
