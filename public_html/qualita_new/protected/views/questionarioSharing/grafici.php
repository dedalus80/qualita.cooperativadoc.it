<?php
$this->breadcrumbs = array('Questionari Sahring' => array('index'), 'statistiche',);
$tmp = Yii::app()->MyUtils->getDatiQuestionario("questionario_sharing");
        ...
        <div class="panel-heading">
            ...
            <div class="panel-ctrls">
                <ul class="demo-btns" >
                    ...
                    <?php endif; ?>
                     <li class='li-480' ><?php echo CHtml::link('<i class="fa fa-refresh"></i>', '#', array('class' => 'button-icon button-icon-green', 'id' => 'stats-sharing-btn', 'rel' => 'tooltip', 'data-toggle' => 'tooltip', 'title' => Yii::t('app', 'Aggiorna dati'))); ?></li>
                    <input type='hidden' id='struttura-utente' name='struttura-utente' value='<?= $model->stats['struttura'] ?>' />
                     ...
                     <input type='hidden' id='tipo-grafico' name='tipo-grafico' value='questionari_sharing' />
...
<?php  if( $model->stats["consiglia"]['totale'] > 0 ){  ?>
    ...
    <?php } foreach($giudizzi as $giudizzio){  
        ...
        series: [{
            type: 'column',
            name: 'Insufficiente',
            data: [<?= implode(",", $model->stats[ $giudizzio ]['I_per']) ?> ],
            color:'#e74c3c',
        }, {
            type: 'column',
            name: 'Sufficiente',
            data: [<?= implode(",", $model->stats[ $giudizzio ]['S_per']) ?>],
            color:'#f1c40f',
        }, {
            type: 'column',
            name: 'Buono',
            data: [<?= implode(",", $model->stats[ $giudizzio ]['B_per']) ?>],
            color:'#35f887',
        },{
            type: 'column',
            name: 'Eccellente',
            data: [<?= implode(",", $model->stats[ $giudizzio ]['E_per']) ?>],
            color:'#0e836c',
        }],
    });
