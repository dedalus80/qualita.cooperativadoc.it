...
$objPHPExcel->getActiveSheet()->getStyle('R1:V1')->applyFromArray($style1);
$objPHPExcel->getActiveSheet()->getStyle('W1:X1')->applyFromArray($style2);


$objPHPExcel->getActiveSheet()->getStyle('A2:X2')->applyFromArray($style4);



for ($x = 2; $x < count($model->datiEsportazione) + 2; $x++) {
    if ($x % 2 == 0)
        $objPHPExcel->getActiveSheet()->getStyle('A' . $x . ':X' . $x)->applyFromArray($style3);
}
...
$objPHPExcel->getActiveSheet()->setCellValue('W1', 'SUGGERIMENTI');
...
$label = array('ID', 'Data restituzione', 'Nome', 'Cognome','Soggiono', 'Turno', 'Organizzazione','Nome gruppo','Nome coordinatore','Cognome coordinatore',
    'Divertimento', 'Educatori', 'Compagni', 'Giochi', 'Attivita Sportive', 'Gite', 'Laboratori',
    'La localitA ', 'Il College',  'Il Corso', 'Aspetto piu apprezato', 'Utilita formativa', 'Suggerimenti','Osservazioni'
);
...
for ($x = 0; $x < count($model->datiEsportazione); $x++) {
    ...
    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(17, $row, Yii::app()->MyUtils->getGiudizio($model->datiEsportazione[$x]['studio_localita'], 3));
    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(18, $row, Yii::app()->MyUtils->getGiudizio($model->datiEsportazione[$x]['studio_college'], 3));
    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(19, $row, Yii::app()->MyUtils->getGiudizio($model->datiEsportazione[$x]['studio_corso'], 3));
    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(20, $row, html_entity_decode($model->datiEsportazione[$x]['studio_aspetto_vacanza']));
    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(21, $row, html_entity_decode($model->datiEsportazione[$x]['studio_attivita_utile']));
    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(22, $row, html_entity_decode($model->datiEsportazione[$x]['suggerimenti']));
    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(23, $row, html_entity_decode($model->datiEsportazione[$x]['osservazioni']));

    $row++;
}

header('Content-Type: application/vnd.ms-excel');
header('Content-Disposition: attachment;filename="Questionari_vacanze_studio.xls"');
