...
Morris.Donut({
 ...
 $(document).ready(function(){
Morris.Donut({
                    element: 'gra-studio_corso',
                    data: [" . implode(",", $model->stats['studio_corso']) . "],
                    colors: [Utility.getBrandColor('danger'),Utility.getBrandColor('warning'),Utility.getBrandColor('success') ]
                });
 ...
 $(document).ready(function(){
Morris.Donut({
                    element: 'gra-studio_college',
                    data: [" . implode(",", $model->stats['studio_college']) . "],
                    colors: [Utility.getBrandColor('danger'),Utility.getBrandColor('warning'),Utility.getBrandColor('success') ]
                });
 ...
 $(document).ready(function(){
Morris.Donut({
                    element: 'gra-studio_localita',
                    data: [" . implode(",", $model->stats['studio_localita']) . "],
                    colors: [Utility.getBrandColor('danger'),Utility.getBrandColor('warning'),Utility.getBrandColor('success') ]
                });
...
<div style="display: <?= $model->stats['totale'] > 0 ? "block" : "none" ?>" id="table-grafici"> 
    ...
    <div class="row">
        <div class="col-xs-12 col-sm-3" style="display: <?= $model->stats['exist']['studio_localita'] > 0 ? "block" : "none" ?>" > 
            <div id="gra-studio_localita"></div>
            <table  class="table  table-bordered dataTable">
                <thead><tr><td colspan="3" class="centered nob-right">Giudizio localit&agrave; corso</td></tr></thead>
                <tbody>
                    <? for ($x = 0; $x < count($model->stats['studio_localita_data']); $x++) { ?>
                        <tr><td ><?= $model->stats['studio_localita_data'][$x]['label'] ?></td><td class='right'><?= $model->stats['studio_localita_data'][$x]['valore'] ?></td><td class='right' ><?= $model->stats['studio_localita_data'][$x]['percentuale'] ?> %</td></tr>
                    <? } ?>
                </tbody>
            </table>
            <div class="graf-label" ></div>
        </div>
        <div class="col-xs-12 col-sm-3" style="display: <?= $model->stats['exist']['studio_college'] > 0 ? "block" : "none" ?>" > 
            <div id="gra-studio_college"></div>
            <table  class="table  table-bordered dataTable">
                <thead><tr><td colspan="3" class="centered nob-right">Giudizio college</td></tr></thead>
                <tbody>
                    <? for ($x = 0; $x < count($model->stats['studio_college_data']); $x++) { ?>
                        <tr><td ><?= $model->stats['studio_college_data'][$x]['label'] ?></td><td class='right'><?= $model->stats['studio_college_data'][$x]['valore'] ?></td><td class='right' ><?= $model->stats['studio_college_data'][$x]['percentuale'] ?> %</td></tr>
                    <? } ?>
                </tbody>
            </table>
        </div>
        <div class="col-xs-12 col-sm-3" style="display: <?= $model->stats['exist']['studio_corso'] > 0 ? "block" : "none" ?>" > 
            <div id="gra-studio_corso"></div>
            <table  class="table  table-bordered dataTable">
                <thead><tr><td colspan="3" class="centered nob-right">Giudizio corso inglese</td></tr></thead>
                <tbody>
                    <? for ($x = 0; $x < count($model->stats['studio_corso_data']); $x++) { ?>
                        <tr><td ><?= $model->stats['studio_corso_data'][$x]['label'] ?></td><td class='right'><?= $model->stats['studio_corso_data'][$x]['valore'] ?></td><td class='right' ><?= $model->stats['studio_corso_data'][$x]['percentuale'] ?> %</td></tr>
                    <? } ?>
                </tbody>
            </table>
        </div>
