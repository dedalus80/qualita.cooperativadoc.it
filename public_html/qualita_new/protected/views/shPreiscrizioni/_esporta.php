...
$objPHPExcel->getProperties()->setCreator("Cooperativa doc")->setTitle("Sharing Preiscrizione");
...
$label = array(
    ...
    'Occupazione', 'Prima Volta Ospite', 'Conosciuti tramite', 'Coabitazione con', 'Consenso', 'Mailing List', 'Note' , 'Anno'
);
...
for ($x = 0; $x < count($model->datiEsportazione); $x++) {
    ...
    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(0, $row, $model->datiEsportazione[$x]['refer'] =='S' ? "Sharing":"Politecnico");
    ...
    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(6, $row, $model->datiEsportazione[$x]['formula'] == '1' ? $model->datiEsportazione[$x]['nome_housing'] : $model->datiEsportazione[$x]['nome_campus'] );
    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(7, $row, $model->datiEsportazione[$x]['nome']);
    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(8, $row, $model->datiEsportazione[$x]['cognome']);
    ...
    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(21, $row, $model->datiEsportazione[$x]['note']);
    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(22, $row, $model->datiEsportazione[$x]['anno']);
    $row++;
}

header('Content-Type: application/vnd.ms-excel');
header('Content-Disposition: attachment;filename="Preiscrizioni_Sharing.xls"');
