...
$objPHPExcel->getActiveSheet()->getStyle('L1:O1')->applyFromArray($style2);
...
for ($x = 2; $x < count($model->datiEsportazione) + 2; $x++) {
    if ($x % 2 == 0)
		...
		$objPHPExcel->getActiveSheet()->getStyle('A' . $x . ':O' . $x)->applyFromArray($style4);
}

$objPHPExcel->getActiveSheet()->setCellValue('A1', 'DATI PARTECIPANTE');
$objPHPExcel->getActiveSheet()->mergeCells('A1:K1');

$objPHPExcel->getActiveSheet()->setCellValue('L1', 'DATI GENITORE');
...
$label = array(
    'ID','Data iscrizione','Ora iscrizione', 'Nome', 'Cognome', 'Codice Fiscale','Iscrizione', 'Soggiorno','Codice turno','Inizio turno','Fine turno','Nome','Cognome','Codice Fiscale','CID'
);
...
for ($x = 0; $x < count($model->datiEsportazione); $x++) {
    ...
    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(6, $row, $model->datiEsportazione[$x]['nome_iscrizione']);
    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(7, $row, $model->datiEsportazione[$x]['nome_soggiorno']);
    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(8, $row, $model->datiEsportazione[$x]['codice_turno']);
    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(9, $row, $model->datiEsportazione[$x]['inizio_turno']);
    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(10, $row, $model->datiEsportazione[$x]['fine_turno']);
    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(11, $row, $model->datiEsportazione[$x]['genitore_nome']);
    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(12, $row, $model->datiEsportazione[$x]['genitore_cognome']);
    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(13, $row, $model->datiEsportazione[$x]['genitore_codice_fiscale']);
    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(14, $row, $model->datiEsportazione[$x]['cid']);
