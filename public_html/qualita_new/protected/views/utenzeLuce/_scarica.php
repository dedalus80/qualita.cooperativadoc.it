...
$objPHPExcel->getActiveSheet()->setTitle("Consumi Energia ");
...
$row = 1;
$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(0, $row, "CONSUMI ENERGIA " . strtoupper($model->struttura_nome) . " " . $model->anno);
$row++;
foreach ($label AS $id => $val)
    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($id, $row, iconv('windows-1251', 'utf-8', $val));

$row++;
$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(0, $row,"Kwh" );
...
$row++;
$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(0, $row,"Kwh / Ospite" );
foreach($mesi as $id => $val){
     $x = "c_".$mese;
...
$row++;
$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(0, $row,"Kwh / Mq" );
foreach($mesi as $id => $val){
     $x = "c_".$mese;
...
header('Content-Disposition: attachment;filename="Consumi_Energetici_'.str_replace(" ","_",$model->struttura_nome).'_'.$model->anno.'.xls"');
