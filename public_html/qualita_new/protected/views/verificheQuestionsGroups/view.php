<?php
/* @var $this VerificheQuestionsGroupsController */
/* @var $model VerificheQuestionsGroups */

$this->breadcrumbs=array(
	'Verifiche Questions Groups'=>array('index'),
	$model->name,
);

$this->menu=array(
	array('label'=>'List VerificheQuestionsGroups', 'url'=>array('index')),
	array('label'=>'Create VerificheQuestionsGroups', 'url'=>array('create')),
	array('label'=>'Update VerificheQuestionsGroups', 'url'=>array('update', 'id'=>$model->id)),
	array('label'=>'Delete VerificheQuestionsGroups', 'url'=>'#', 'linkOptions'=>array('submit'=>array('delete','id'=>$model->id),'confirm'=>'Are you sure you want to delete this item?')),
	array('label'=>'Manage VerificheQuestionsGroups', 'url'=>array('admin')),
);
?>

<h1>View VerificheQuestionsGroups #<?php echo $model->id; ?></h1>

<?php $this->widget('zii.widgets.CDetailView', array(
	'data'=>$model,
	'attributes'=>array(
		'id',
		'tipologiaVerificaId',
		'name',
		'rank',
	),
)); ?>
