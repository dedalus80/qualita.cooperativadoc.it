<script>
    ...
    function updatePeriodo(){
        ...
        if(anno !=0 && mese!=0)
            window.location.href = 'http://qualita.cooperativadoc.it/qualita/index.php/questionarioSharing/create/periodo/'+anno+'-'+mese;
    }
...
foreach ($model->attributes as $id => $val)
    $stats[$id] = $model->getStats($id,$periodo);

$consiglia =$model->getConsiglia($model->attributes['consiglia'],$periodo);
    ...
    <table cellspacing='0' cellpadding='0'>
        <tr>
            <td class="grey  text-left" colspan="5" style="border-top: #CCC 1px solid">
                <br>
                Scegli periodo
                <select name="mesi" id="mesi" onChange="updatePeriodo()" class="myslim">
                <?
                ...
                <select name="anni" id="anni" onChange="updatePeriodo()" class="myslim" >
